<script lang="ts">
  ...
  import { Tv, Plus, ShieldAlert, CheckCircle2, ChevronRight, Search, Trophy, Settings } from 'lucide-svelte';
...
<div class="space-y-6">
  ...
  <!-- Active Games List (Enforces clean, non-nested flex-row structures) -->
  <div class="space-y-3">
    ...
    {:else}
      <div class="space-y-2">
        {#each filteredGames as game}
          <div class="flex flex-col sm:flex-row sm:items-center justify-between border border-neutral-800/80 bg-neutral-900/40 p-4 rounded-xl gap-4 hover:border-neutral-700 transition">
            
            <!-- Left Side: Interactive link to the standard Odds Editor -->
            <a href="/admin/games/{game.id}" class="flex items-center gap-4 flex-1 group">
              <div class="rounded-lg bg-neutral-950 p-2.5 border border-neutral-800 text-neutral-400 group-hover:text-red-500 transition">
                <Tv class="h-4 w-4" />
              </div>
              <div>
                <div class="text-xs font-bold text-neutral-200 group-hover:text-white transition">{game.homeTeam} vs {game.awayTeam}</div>
                <div class="text-[10px] text-neutral-500 mt-1">{game.league} &bull; Ref: {game.id} &bull; Kickoff {formatGameTime(game.startTime)}</div>
              </div>
            </a>

            <!-- Right Side: Status Badge, direct Settle Shortcut button, and Chevron -->
            <div class="flex items-center gap-3 self-end sm:self-auto shrink-0">
              ...
              {#if game.status !== 'COMPLETED'}
                <!-- Direct Payout Portal Settle Shortcut (Bypasses the odds editor page) -->
                <a href="/admin/games/{game.id}/settle">
                  <Button class="h-8 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-black rounded gap-1.5 px-3 shadow-md cursor-pointer">
              ...
              <a href="/admin/games/{game.id}" class="text-neutral-500 hover:text-white transition">
