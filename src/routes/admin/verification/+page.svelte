  ...
  <div class="space-y-3">
    <h2 class="text-xs font-black tracking-widest text-red-400 uppercase flex items-center gap-1.5">
      ...
      {#if pendingDocs.length > 0}
        <span class="rounded-full bg-red-600 px-2 py-0.5 text-[10px] font-black text-foreground">{pendingDocs.length}</span>
      {/if}
    </h2>
    ...
    {:else}
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        {#each pendingDocs as doc}
          <div class="rounded-xl border border-border bg-background/40 p-5 flex flex-col justify-between gap-4">
            <div class="space-y-3">
              <!-- Card Header -->
              <div class="flex justify-between items-start">
                ...
                <a href={doc.fileUrl} target="_blank" class="rounded bg-background p-2 border border-border text-neutral-500 hover:text-foreground transition">
                  <Eye class="h-4 w-4" />
                </a>
              </div>

              <!-- Inline Document Preview Image container -->
              <div class="rounded-lg overflow-hidden border border-border/80 bg-black/40 h-40 flex items-center justify-center relative">
        ...
        <tbody class="divide-y divide-neutral-800 font-medium">
          {#each data.documents as doc}
            <tr class="hover:bg-background/10 transition duration-150">
              <td class="whitespace-nowrap px-4 py-4 text-foreground font-bold">{doc.profile.username}</td>
              <td class="whitespace-nowrap px-4 py-4 font-bold">{doc.documentType.replace('_', ' ')}</td>
              <td class="whitespace-nowrap px-4 py-4 text-muted-foreground">{formatGameTime(doc.submittedAt)}</td>
