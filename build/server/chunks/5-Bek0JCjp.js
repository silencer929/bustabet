...
var load = async () => {
	...
	const [openTicketsRaw] = await db.execute("SELECT COUNT(*) AS openTicketsCount FROM supportConversations WHERE status = ?", ["OPEN"]);
	const openTicketsCount = openTicketsRaw[0].openTicketsCount;
	const [pendingKycRaw] = await db.execute("SELECT COUNT(*) AS pendingKycCount FROM verificationDocs WHERE status = ?", ["PENDING"]);
	...
	const [settledPayoutsRaw] = await db.execute("SELECT SUM(potentialWin) AS totalPayouts FROM bets WHERE status = ?", ["WON"]);
...
//# sourceMappingURL=5-Bek0JCjp.js.map
