...
var load = async ({ params, locals }) => {
	...
	const [games] = await db.execute(`SELECT id, sport, league, home_team as homeTeam, away_team as awayTeam, home_score, away_score, start_time as startTime, status 
     FROM admin_games 
		...
		try {
         ...
         WHERE b.game_id = ? AND b.status = 'PENDING'`, [params.id]);
			...
			for (const bet of pendingBets) {
				...
				else isWon = homeScore > awayScore && sel.includes(game.home_team.toUpperCase()) || awayScore > homeScore && sel.includes(game.away_team.toUpperCase());
			...
			return {
				success: true,
				message: "Game score registered and all single and combo wagers settled successfully!"
			};
		} catch (error) {
			await conn.rollback();
