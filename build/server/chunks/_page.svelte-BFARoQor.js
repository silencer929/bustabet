...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		Arrow_down_left($$renderer, { class: "h-5 w-5 text-amber-500" });
		$$renderer.push(`<!----> Deposit Funds</h2> <p class="text-xs text-neutral-500 font-semibold">Initiate an instant Safaricom M-Pesa STK push deposit</p></div> `);
		if (form?.success) {
			$$renderer.push("<!--[0-->");
			...
			$$renderer.push(`<!--]--> <div class="space-y-1.5"><label for="amount" class="text-xs font-bold text-muted-foreground">Amount to Deposit</label> <div class="relative">`);
			Input($$renderer, {
				id: "amount",
				name: "amount",
				type: "number",
			...
			$$renderer.push(`<!----> <span class="absolute right-3 top-2.5 text-xs font-bold text-neutral-500">${escape_html(auth.user?.currency || "KES")}</span></div></div> <div class="space-y-1.5"><label for="phone" class="text-xs font-bold text-muted-foreground">M-Pesa Mobile Number</label> `);
			Input($$renderer, {
				id: "phone",
				name: "phone",
				type: "tel",
			...
			$$renderer.push(`<!----> <span class="text-[10px] text-neutral-500 block">Please enter in international E.164 format (+254...)</span></div> `);
			Button($$renderer, {
				type: "submit",
				disabled: isSubmitting,
...
//# sourceMappingURL=_page.svelte-BFARoQor.js.map
