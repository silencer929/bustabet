...
import { S as SettlementService } from './settlement.service-BGawB5fp.js';
	...
	static async syncLiveScores(sportKey) {
		...
		try {
			for (const match of scores) {
				...
				const [existing] = await conn.execute("SELECT id FROM admin_games WHERE id = ? LIMIT 1", [match.id]);
...
export { GET };
//# sourceMappingURL=_server.ts-CoQlHy6Q.js.map
