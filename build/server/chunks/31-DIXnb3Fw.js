...
import { O as OddsSyncService } from './oddsSync.service-7Hn4Bxe1.js';
import { r as redirect, e as error } from './index-BQZSrJq2.js';
...
var load = async ({ params, locals }) => {
	...
	const [games] = await db.execute("SELECT id, sport FROM admin_games WHERE id = ? LIMIT 1", [params.id]);
	...
	const [gameRaw] = await db.execute(`SELECT id, sport, league, home_team as homeTeam, away_team as awayTeam, start_time as startTime, status 
     FROM admin_games 
